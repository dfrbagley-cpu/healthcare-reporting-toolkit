...
async function verifyMonthlyReport(page) {
  ...
  await page.addScriptTag({ url: `${server.url}/__test__/axe.min.js` });
  assert.equal(await page.locator("#mode-synthetic").isChecked(), true);
  assert.equal(await page.locator("#baseline").inputValue(), "");
  await page.locator("#synthetic-baseline").selectOption("baseline.csv");
  await page.locator("#synthetic-current").selectOption("current.csv");
  assert.equal(await page.locator("#baseline").evaluate((el) => el.files[0].name), "baseline.csv");
  ...
  assert.match(await page.locator("#reconciliation").textContent(), /Baseline eligible events800/);
  assert.match(await page.locator("#reconciliation").textContent(), /Current eligible events794/);
  ...
  assert.equal(summary.reconciliation.delta, -6);
  assert.equal(summary.evidence.mode, "synthetic");
  assert.equal(summary.evidence.synthetic_dataset, "monthly-clinics-1.0.0");
  ...
  await page.locator('[data-download-file="current-duplicate.csv"]').click();
  const downloaded = await downloadEvent;
  assert.equal(downloaded.suggestedFilename(), "current-duplicate.csv");
  await page.locator("#current").setInputFiles(await downloaded.path());
  await page.locator("#current-complete").check();
  await page.locator("#run").click();
  ...
  // A filename alone cannot label arbitrary data as synthetic.
  await page.locator("#synthetic-baseline").selectOption("baseline.csv");
  await page.locator("#current").setInputFiles({ name: "current.csv", mimeType: "text/csv", buffer: Buffer.from("site,event_id,service_on,status\nN,own,2026-08-01,completed\n") });
  await page.locator("#baseline-complete").check(); await page.locator("#current-complete").check();
  await page.locator("#run").click();
  await page.waitForFunction(() => document.querySelector("#error").textContent.includes("not an unchanged supplied synthetic"));
  await page.locator("#mode-open").check();
  assert.equal(await page.locator("#synthetic-library").isVisible(), false);
  assert.equal(await page.locator("#synthetic-pickers").isVisible(), false);
  assert.equal(await page.locator("#baseline").inputValue(), "");
  assert.equal(await page.locator("#current").inputValue(), "");
  assert.equal(await page.locator("#keys").inputValue(), "");
  assert.equal(await page.locator("#start").inputValue(), "");
  await page.locator("#keys").fill("site,event_id");
  for (const [id, value] of [["start", "2026-08-01"], ["end", "2026-08-31"], ["baseline-extracted", "2026-09-01"], ["current-extracted", "2026-09-07"]]) await page.locator(`#${id}`).fill(value);
  ...
  const openSummaryDownload = page.waitForEvent("download"); await page.locator("#save-summary").click();
  const openSummary = JSON.parse(await readFile(await (await openSummaryDownload).path(), "utf8"));
  assert.equal(openSummary.evidence.mode, "open"); assert.equal(openSummary.evidence.synthetic_dataset, null);
  await verifyAccessibility(page, "monthly-report-open");
  await page.locator("#mode-synthetic").check();
  assert.equal(await page.locator("#results").isVisible(), false);
  assert.equal(await page.locator("#baseline").inputValue(), "");
  assert.equal(await page.locator("#current").inputValue(), "");
  assert.equal(await page.locator("#baseline-complete").isChecked(), false);
...
async function verifyReceiptJourney(page, reportingWindowReceipt) {
  ...
  assert.equal(await page.locator("#receipt-version").textContent(), "v0.8.0");
